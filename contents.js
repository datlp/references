...
document.querySelectorAll('.verse>.label').forEach(item=>{item.innerText=`<sup id="${hashBible[vid]}.${item.parentNode.dataset.usfm}">${item.innerText}</sup>`})
document.querySelectorAll('.chapter>.label').forEach(item=>{item.innerText=`<chapter id="${hashBible[vid]}.${item.parentNode.dataset.usfm}"></chapter>`})
...
text = result.books.map(item=>(item.contents.map((item2,contentStatus)=>`${hashBible[result.vid]}\t${hashBible[result.vid]}.${item.id}\t${item.status}\t${item.id}\t${contentStatus}\t${item2.replaceAll('\n','<br/>').replaceAll(/\t/g,'')}`)).join('\n')).join('\n')
